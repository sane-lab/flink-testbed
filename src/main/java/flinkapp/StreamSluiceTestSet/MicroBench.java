...
public class MicroBench {
    public static void main(String[] args) throws Exception {
        ...
        if (SOURCE_TYPE.equals("changing_amplitude_and_period")) {
        ...
        } else if(SOURCE_TYPE.equals("part4")){
            String PHASE_TIMES = params.get("phaseTimes"), PHASE_RATES = params.get("phaseRates"), PHASE_AMPLITUDES = params.get("phaseAmplitudes"), PHASE_PERIODS = params.get("phasePeriods"), PHASE_CURVE_TYPES = params.get("phaseTypes");
            source = env.addSource(new MultiplePhaseSource(params.getLong("warmupTime", 20) * 1000, params.getLong("warmupRate", INTERMEDIATE_RATE), params.getLong("run_time", 510) * 1000,
                            PHASE_TIMES, PHASE_RATES, PHASE_AMPLITUDES, PHASE_PERIODS, PHASE_CURVE_TYPES,
            ...
            long amplitude_low = params.getLong("amplitudeLow", 5000);
            long amplitude_high = params.getLong("amplitudeHigh", 5000);
            source = env.addSource(new SystemSensitivityWithNoiseSource(params.getLong("warmupTime", 20) * 1000,
                    params.getLong("warmupRate", INTERMEDIATE_RATE),
                    params.getLong("run_time", 510) * 1000,
                    params.get("curve_type", "sine"),
        ...
        SingleOutputStreamOperator<Tuple3<String, Long, Long>> leng3 = leng2.keyBy(0)
                .flatMap(new DumbStatefulMap(params.getLong("op4Delay", 100), params.getInt("op4IoRate", 1), params.getBoolean("op4IoFix", true), params.getInt("op4KeyStateSize", 1)))
                .disableChaining()
                .name("FlatMap 4")
                .uid("op4")
        ...
        if(GRAPH_TYPE.equals("4op")){
            leng3.keyBy(0).map(new DumbSink(params.getLong("op5Delay", 100), params.getInt("op5KeyStateSize", 1), params.getBoolean("outputGroundTruth", true)))
                    .disableChaining()
                    .name("FlatMap 5")
                    .uid("op5")
                    .setParallelism(params.getInt("p5", 1))
    ...
    public static final class MultiplePhaseSource implements SourceFunction<Tuple3<String, Long, Long>>, CheckpointedFunction {
        final private long WARMP_TIME, WARMP_RATE, TOTAL_TIME;
        final private ArrayList<Long> PHASE_TIMES, PHASE_RATES, PHASE_AMPLITUDES, PHASE_PERIODS, PHASE_TYPES;
        ...
        private static long getCurveTypeId(String PATTERN){
            switch(PATTERN) {
                case "gradient":
                    return 0;
                case "linear":
        ...
        private static ArrayList<Long> parsePhaseTypesString(String phase_string){
            String[] parts = phase_string.split("-");
            ArrayList<Long> phase_values = new ArrayList<>();
            for (String part : parts) {
                phase_values.add(getCurveTypeId(part));
            }
            return phase_values;
        }

        public MultiplePhaseSource(long WARMUP_TIME, long WARMUP_RATE, long TOTAL_TIME,
                                   String phase_times_str, String phase_rates_str, String phase_amplitudes_str,
                                   String phase_periods_str, String phase_types_str,
                                   double NOISE_LEVEL, double zipf_skew){
        ...
        private void generateCurve(SourceContext<Tuple3<String, Long, Long>> ctx) throws Exception {
            long startTime = System.currentTimeMillis();
            System.out.println("Phase Rates: " + this.PHASE_RATES
                    + "\nPhase Times: " + this.PHASE_TIMES
                    + "\nPhase Amplitudes: " + this.PHASE_AMPLITUDES
                    + "\nPhase Periods: " + this.PHASE_PERIODS
                    + "\nPhase Types: " + this.PHASE_TYPES
            ...
            while (isRunning && System.currentTimeMillis() < endTime) {
                ...
                long now_average_rate = PHASE_RATES.get((int) (phase_index % PHASE_RATES.size()));
                long now_phase_time = PHASE_TIMES.get((int) (phase_index % PHASE_TIMES.size()));
                long now_amplitude = PHASE_AMPLITUDES.get((int) (phase_index % PHASE_AMPLITUDES.size()));
                long now_period = PHASE_PERIODS.get((int) (phase_index % PHASE_PERIODS.size()));
                long phase_pattern = PHASE_TYPES.get((int) (phase_index % PHASE_TYPES.size()));
                long phaseEndTime = Math.min(phaseStartTime + now_phase_time, endTime);
                long round = 0;
                while (isRunning && System.currentTimeMillis() < phaseEndTime) {
                    long pattern_this_round;
                    long roundStartTime = System.currentTimeMillis();
                    if (phase_pattern != 4) {
                        pattern_this_round = phase_pattern;
                    } else {
                        pattern_this_round = round % 4;
                    }
                    if (pattern_this_round == 0) {
                        ...
                        roundStartTime = System.currentTimeMillis();
                        startSteadyPhase(ctx, now_average_rate, 90000 - now_period, roundStartTime);
    ...
    public static final class AverageRateChangeAmplitudeChangeWithNoiseSource implements SourceFunction<Tuple3<String, Long, Long>>, CheckpointedFunction {
        ...
        public AverageRateChangeAmplitudeChangeWithNoiseSource(long WARMUP_TIME, long WARMUP_RATE, long TOTAL_TIME, String PATTERN,
            ...
            switch(AVERAGE_RATE_PATTERN){
                ...
                case "sine":
                    ...
                    break;
                default:
                    if(AVERAGE_RATE_PATTERN.startsWith(prefix)){
                        this.AVERAGE_RATE_PATTERN = Integer.parseInt(AVERAGE_RATE_PATTERN.substring(prefix.length()));
                    }else {
                        this.AVERAGE_RATE_PATTERN = 1;
                    }
                    break;
            ...
            switch(AMPLITUDE_PATTERN){
                ...
                case "sine":
                    ...
                    break;
                default:
                    if(AMPLITUDE_PATTERN.startsWith(prefix)){
                        this.AMPLITUDE_PATTERN = Integer.parseInt(AMPLITUDE_PATTERN.substring(prefix.length()));
                    }else {
                        this.AMPLITUDE_PATTERN = 1;
                    }
                    break;
            ...
            switch(PERIOD_PATTERN){
                ...
                case "sine":
                    ...
                    break;
                default:
                    if(PERIOD_PATTERN.startsWith(prefix)){
                        this.PERIOD_PATTERN = Integer.parseInt(PERIOD_PATTERN.substring(prefix.length()));
                    }else {
                        this.PERIOD_PATTERN = 1;
                    }
                    break;
        ...
        private long calculateValueAtCurrentTime(long time, long low, long high, long period, long pattern){
            ...
            time = time % period;
            if(pattern == 0){
                long period_four = period / 4;
                if(time < period_four){
                    value = (low + high) / 2;
                }else if(time < period_four * 2){
                    value = high;
                }else if(time < period_four * 3){
                    value = (low + high) / 2;
                }else{
                    value = low;
                }
            }else if(pattern == 1){
                long baseRate = (low + high) / 2;
                long period_four = period / 4;
                if(time < period_four){
                    value = (long) ((high - baseRate) * (time / (double) period_four) + baseRate);
                }else if(time < period_four * 2){
                    value = (long) (high - (high - baseRate) * ((time - period_four) / (double) period_four));
                }else if(time < period_four * 3){
                    value = (long) (baseRate - (baseRate - low) * ((time - period_four * 2) / (double) period_four));
                }else{
                    value = (long) ((baseRate - low) * ((time - period_four * 3) / (double) period_four) + low);
                }
            }else if(pattern == 2){
                ...
                if(time < period / 2){
                    ...
                    value = high - stair_rate * ((time - period / 2) / period_n);
                }
            }else{
                value = (low + high) / 2;
            }
    ...
    private void generateCurve(SourceContext<Tuple3<String, Long, Long>> ctx) throws Exception {
        long startTime = System.currentTimeMillis();
        System.out.println("Rate paras: " + "\nAmplitude paras: " + this.AMPLITUDE_LOW + ", " + this.AMPLITUDE_HIGH
                + "\nPeriod pattern: " + this.PERIOD_LOW + ", " + this.PERIOD_HIGH + ", "
                + "\nNoise: " + this.NOISE_LEVEL);
        System.out.println("Source start at: " + startTime);
        System.out.println("Source warm up...");
        startSteadyPhase(ctx, WARMP_RATE, WARMP_TIME, startTime);
        ...
        while (isRunning && System.currentTimeMillis() - startTime < TOTAL_TIME) {
            ...
            if (pattern_this_round == 0){
                ...
                roundStartTime = System.currentTimeMillis();
                startLinearPhase(ctx, now_average_rate + now_amplitude, now_average_rate, now_period / 2, roundStartTime);
            }else if (pattern_this_round == 2) {
                System.out.println("Round " + round + " sine phase start at: " + roundStartTime);
                System.out.println("phase paras: " + now_average_rate + ", " + now_amplitude + ", " + now_period);
                startSteadyPhase(ctx, now_average_rate, 30000, roundStartTime);
