...
source config-mlscore-local.sh
...
run_one_exp() {
    EXP_NAME=part8-ml-${controller_type}-${metrics_report_interval}-${runtime}-${warmup_time}-${warmup_rate}-${skip_interval}-${P2}-${DELAY2}-${P3}-${DELAY3}-${P4}-${DELAY4}-${P5}-${DELAY5}-${L}-${autotuner_increase_bar_alpha}-${epoch}-${input_rate_factor}-${PAYLOAD}-${SKEWNESS}-${is_treat}-${migration_interval}-${conservative_factor}-${repeat}
...
init() {
  ...
  # set in Flink app
  stock_path="/home/samza/ML_data/"
  stock_file_name="ml-4hr-50ms.txt"
  MP1=1
  MP2=128
  MP3=128
  MP4=128
