...
function configFlink() {
    ...
    sed 's/^\(\s*controller.scale_in.type\s*:\s*\).*/\1'"$scalein_type"'/' tmp23 > ${FLINK_CONF_DIR}/flink-conf.yaml
    rm tmp*
    ...
    for host in "dragon" "eagle"; do
      ...
      scp ${FLINK_CONF_DIR}/flink-conf-slave-systemsensitivity ${host}:${FLINK_DIR}/conf/flink-conf.yaml
    done
}
