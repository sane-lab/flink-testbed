...
run_one_exp() {
  EXP_NAME=microbench-system-server-${whether_type}-${how_type}-${CURVE_TYPE}-${GRAPH}-${runtime}-${RATE1}-${RATE2}-${RATE_I}-${RANGE_I}-${PERIOD_I}-${P1}-${ZIPF_SKEW}-${P2}-${DELAY2}-${IO2}-${STATE_SIZE2}-${P3}-${DELAY3}-${IO3}-${STATE_SIZE3}-${P4}-${DELAY4}-${IO4}-${STATE_SIZE4}-${L}-${migration_interval}-${epoch}-${decision_interval}-${is_treat}-${repeat}
...
run_scale_test(){
    ...
    #STATE_SIZE4=1000
    spike_intercept=500
    STATE_SIZE2=2000 #10000
    STATE_SIZE3=2000 #10000
    STATE_SIZE4=2000 #10000
    runtime=60
    DELTA_I=270
    LP2=5
    LP3=5
...
#    #for how_type in "op_1_6_keep" "op_1_1_keep" "op_2_2_keep"; do
#    for how_type in "streamsluice_minus_one"  "streamsluice_no_balance" "streamsluice_more" "streamsluice_less" "streamsluice_not_bottleneck"; do
#      run_one_exp
#      printf "${EXP_NAME}\n" >> whetherhow_result.txt
#    done
