...
function configFlink() {
    ...
    sed 's/^\(\s*streamsluice.system.conservative\s*:\s*\).*/\1'"$conservative_factor"'/' tmp40 > tmp41
    sed 's/^\(\s*controller.whether.option\s*:\s*\).*/\1'"$scaling_decision_option"'/' tmp41 > tmp42
