...
run_one_exp() {
  EXP_NAME=algorithm_test-${scaling_decision_option}-${coordination_latency_flag}-${whether_type}-${how_type}-${conservative_service_rate_flag}-${smooth_backlog_flag}-${SOURCE_TYPE}-${CURVE_TYPE}-${GRAPH}-${runtime}-${RATE1}-${TIME1}-${RATE2}-${RATE_I}-${TIME_I}-${P1}-${ZIPF_SKEW}-${P2}-${DELAY2}-${IO2}-${STATE_SIZE2}-${P3}-${DELAY3}-${IO3}-${STATE_SIZE3}-${P4}-${DELAY4}-${IO4}-${STATE_SIZE4}-${P5}-${DELAY5}-${STATE_SIZE5}-${L}-${migration_interval}-${epoch}-${decision_interval}-${is_treat}-${repeat}
...
#    printf "${EXP_NAME}\n" >> whetherhow_result.txt
    is_treat=true
    how_type="streamsluice"
    for scaling_decision_option in 1 2; do # 0
