...
function configFlink() {
    ...
    sed 's/^\(\s*model.max_parallelism.8e0d1d377d577c52511ad507bf0ce330\s*:\s*\).*/\1'"$LP8"'/' tmp23 > tmp24
    sed 's/^\(\s*model.max_parallelism.2019e6125f7b4867f5fd448be51e3519\s*:\s*\).*/\1'"$LP9"'/' tmp24 > tmp25
    sed 's/^\(\s*streamsluice.model.decision_interval\s*:\s*\).*/\1'"$decision_interval"'/' tmp25 > tmp26
    sed 's/^\(\s*streamsluice.metrics.snapshot_size\s*:\s*\).*/\1'"$snapshot_size"'/' tmp26 > tmp27
    sed 's/^\(\s*streamsluice.system.errorcase_number\s*:\s*\).*/\1'"$errorcase_number"'/' tmp27 > tmp28
    sed 's/^\(\s*controller.scale_in.type\s*:\s*\).*/\1'"$scalein_type"'/' tmp28 > tmp29
    sed 's/^\(\s*streamsluice.system.true_arrival_calibrated\s*:\s*\).*/\1'"$calibrate_selectivity"'/' tmp29 > ${FLINK_CONF_DIR}/flink-conf.yaml
