...
# configure parameters in flink bin - ML Scoring Job specific (local version)
function configFlink() {
    ...
    sed 's/^\(\s*policy.windowSize\s*:\s*\).*/\1'"$metrics_report_interval"'/' tmp3 > tmp3a
    sed 's/^\(\s*streamsluice.system.metrics_interval\s*:\ss*\).*/\1'"$epoch"'/' tmp3a > tmp4
    ...
    # Add metrics.report.flag modification
    sed 's/^\(\s*metrics.report.flag\s*:\s*\).*/\1'"$metrics_report"'/' tmp12 > tmp12b
    sed 's/^\(\s*streamsluice.system.spike_estimation\s*:\s*\).*/\1'"$spike_estimation"'/' tmp12b > tmp13
    ...
    sed 's/^\(\s*controller.how.optimizaiton\s*:\s*\).*/\1'"$how_optimization_flag"'/' tmp23 > tmp24
    sed 's/^\(\s*controller.how.conservative\s*:\s*\).*/\1'"$how_conservative_flag"'/' tmp24 > tmp25
    sed 's/^\(\s*controller.intrinsic_bound\s*:\s*\).*/\1'"$how_intrinsic_bound_flag"'/' tmp25 > tmp26
    sed 's/^\(\s*model.coordination_latency_flag\s*:\s*\).*/\1'"$coordination_latency_flag"'/' tmp26 > tmp27
    sed 's/^\(\s*model.conservative_service_rate_flag\s*:\s*\).*/\1'"$conservative_service_rate_flag"'/' tmp27 > tmp28
    sed 's/^\(\s*streamsluice.system.backlog_smooth\s*:\s*\).*/\1'"$smooth_backlog_flag"'/' tmp28 > tmp29
    sed 's/^\(\s*streamsluice.metrics.use_new_retriever\s*:\s*\).*/\1'"$new_metrics_retriever_flag"'/' tmp29 > tmp30
    sed 's/^\(\s*streamsluice.system.autotuner\s*:\s*\).*/\1'"$autotuner"'/' tmp30 > tmp31
    sed 's/^\(\s*streamsluice.system.autotune.latency_window\s*:\s*\).*/\1'"$autotuner_latency_window"'/' tmp31 > tmp32
    sed 's/^\(\s*streamsluice.system.autotune.bar_lowerbound\s*:\s*\).*/\1'"$autotuner_bar_lowerbound"'/' tmp32 > tmp33
    sed 's/^\(\s*streamsluice.system.autotune.initial_value_option\s*:\s*\).*/\1'"$autotuner_initial_value_option"'/' tmp33 > tmp34
    sed 's/^\(\s*streamsluice.system.autotune.adjustment_option\s*:\s*\).*/\1'"$autotuner_adjustment_option"'/' tmp34 > tmp35
    sed 's/^\(\s*streamsluice.system.autotune.increase_bar_option\s*:\s*\).*/\1'"$autotuner_increase_bar_option"'/' tmp35 > tmp36
    ...
    sed 's/^\(\s*model.transmission_delay\s*:\s*\).*/\1'"$transmission_delay"'/' tmp42 > tmp43
    
    # ML Scoring Job specific configurations for local deployment
    sed 's/^\(\s*mlscore.sine.baseline\s*:\s*\).*/\1'"$ml_sine_baseline"'/' tmp43 > tmp44
    sed 's/^\(\s*mlscore.sine.amplitude\s*:\s*\).*/\1'"$ml_sine_amplitude"'/' tmp44 > tmp45
    sed 's/^\(\s*mlscore.sine.period\s*:\s*\).*/\1'"$ml_sine_period"'/' tmp45 > tmp46
    sed 's/^\(\s*mlscore.spike.probability\s*:\s*\).*/\1'"$ml_spike_probability"'/' tmp46 > tmp47
    sed 's/^\(\s*mlscore.spike.multiplier\s*:\s*\).*/\1'"$ml_spike_multiplier"'/' tmp47 > tmp48
    sed 's/^\(\s*mlscore.fluctuation.std\s*:\s*\).*/\1'"$ml_fluctuation_std"'/' tmp48 > tmp49
    sed 's/^\(\s*mlscore.parse.delay\s*:\s*\).*/\1'"$ml_parse_delay"'/' tmp49 > tmp50
    sed 's/^\(\s*mlscore.feature.delay\s*:\s*\).*/\1'"$ml_feature_delay"'/' tmp50 > ${FLINK_CONF_DIR}/flink-conf.yaml
...
function cleanEnv() {
    ...
    ${KAFKA_PATH}/kafka-topics.sh --delete --zookeeper localhost:2181 --topic ml_scoring_results;
    ...
    ${KAFKA_PATH}/kafka-topics.sh --create --zookeeper localhost:2181 --topic ml_scoring_results --partitions 1 --replication-factor 1;
