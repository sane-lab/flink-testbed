...
function configFlink() {
    ...
    sed 's/^\(\s*model.use_DP_algorithm_flag\s*:\s*\).*/\1'"$lem_dp_algorithm_flag"'/' tmp43 > tmp44
    sed 's/^\(\s*controller.scale_in.type\s*:\s*\).*/\1'"$scalein_type"'/' tmp44 > ${FLINK_CONF_DIR}/flink-conf.yaml
