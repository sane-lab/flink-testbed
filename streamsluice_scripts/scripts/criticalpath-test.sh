...
run_one_exp() {
  EXP_NAME=streamsluice-dag-${runtime}-${RATE1}-${RATE2}-${RATE_I}-${PERIOD_I}-${P1}-${ZIPF_SKEW}-${P2}-${DELAY2}-${IO2}-${STATE_SIZE2}-${P3}-${DELAY3}-${STATE_SIZE3}-${P4}-${DELAY4}-${STATE_SIZE4}-${L}-${migration_overhead}-${epoch}-${is_treat}-${repeat}
...
init() {
  ...
  job="flinkapp.StreamSluiceTestSet.CriticalPathTest"
  # only used in script
  runtime=120
  # set in Flink app
  RATE1=300
