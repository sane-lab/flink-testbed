...
run_one_exp() {
  EXP_NAME=part7-${setting}-${whether_type}-${how_type}-${autotuner_initial_value_alpha}-${SOURCE_TYPE}-${CURVE_TYPE}-${GRAPH}-${runtime}-${rate_low}-${rate_high}-${rate_period}-${rate_pattern}-${amplitude_low}-${amplitude_high}-${amplitude_period}-${amplitude_pattern}-${period_low}-${period_high}-${period_period}-${period_pattern}-${P1}-${ZIPF_SKEW}-${P2}-${DELAY2}-${IO2}-${STATE_SIZE2}-${P3}-${DELAY3}-${IO3}-${STATE_SIZE3}-${P4}-${DELAY4}-${IO4}-${STATE_SIZE4}-${P5}-${DELAY5}-${STATE_SIZE5}-${noise}-${autotuner_increase_bar_alpha}-${L}-${migration_interval}-${epoch}-${decision_interval}-${is_treat}-${repeat}
...
#            printf "${EXP_NAME}\n" >> part7_result.txt
    ...
    for CURVE_TYPE in "linear"; do # "linear" "quarter-sine" "gradient"
      for period_low in 80; do # 120
        for ZIPF_SKEW in 0.1 0.2; do # 0.3
