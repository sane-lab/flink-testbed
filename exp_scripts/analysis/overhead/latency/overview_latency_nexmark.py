...
def ReadFile(query_id, query_name):
    x_axis = []
    y_axis = []
    ...
    for pair in reading_files:
        ...
        for tid in range(0, 2):
            f = open(utilities.FILE_FOLER + "/spector-nexmark-query{}-{}-{}/{}-{}.output"
...
if __name__ == "__main__":
    for (query_id, query_name) in [(1, "Mapper"), (2, "Splitter FlatMap"), (5, "window"), (8, "join")]:
        x_axis, y_axis = ReadFile(query_id, query_name)
        legend_labels = ["Once", "Fluid", "Replication"]
        legend = True
        DrawFigure(x_axis, y_axis, legend_labels, "Time (s)", "Latency (ms)", "state_migration_nexmark_q{}".format(query_id), legend)
