...
def ReadFile():
    ...
    pairs = [(128, 2, "Shuffle-2"), (256, 4, "Shuffle-4"), (512, 8, "Shuffle-8")]
    ...
    for pair in pairs:
        ...
        affected_keys = pair[0]
        affected_tasks = pair[1]
        col = []
        coly = []
        start_ts = float('inf')
        temp_dict = {}
        ...
        for tid in range(0, 8):
            ...
            if not os.path.isfile(FILE_FOLER + "/spector-{}-{}-{}-{}-{}/Splitter FlatMap-{}.output"
                        .format(per_key_state_size, sync_keys, replicate_keys_filter, affected_keys, affected_tasks, tid)):
               continue 
            f = open(FILE_FOLER + "/spector-{}-{}-{}-{}-{}/Splitter FlatMap-{}.output"
                        .format(per_key_state_size, sync_keys, replicate_keys_filter, affected_keys, affected_tasks, tid))
            read = f.readlines()
    ...
    for i in range(w):
        ...
        completion_time_dict[pairs[i][1]] = completion_time
    ...
    x_axis.append(["Shuffle-2", "Shuffle-4", "Shuffle-8"])
    x_axis.append(["Shuffle-2", "Shuffle-4", "Shuffle-8"])
    x_axis.append(["Shuffle-2", "Shuffle-4", "Shuffle-8"])
...
def DrawFigure(xvalues, yvalues, legend_labels, x_label, y_label, y_label_2, filename, allow_legend):
    ...
    plt.xticks(x_values[0], ["Shuffle-2", "Shuffle-4", "Shuffle-8"])
    # plt.grid()
...
if __name__ == "__main__":
    ...
    legend = False
    DrawFigure(x_axis, y_axis, legend_labels, "Shuffled Tasks", "Completion Time (ms)", "P99 Latency Spike (ms)", "pareto_curve_multisource", legend)
