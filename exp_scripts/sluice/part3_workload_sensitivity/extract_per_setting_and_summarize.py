...
def draw_parallelism_curve(rawDir, outputDir, exp_name, windowSize, startTime, exp_length, draw_parallelism_flag) -> [float, [list[int], list[float]]]:
    ...
    if max(ay) <= 16000:
        ...
        ax2.set_yticks(np.arange(4000, 18000, 2000))
    else:
    ...
    exps_per_label_per_setting = {
        "setting_1":{
            "1000": [
                ...
                "setting1--streamsluice-streamsluice-false-true-false-when-sine-1split2join1-720-12500-90-7500-10000-0-1-0-1-20-1-5000-1-20-1-5000-1-20-1-5000-17-500-5000-1000-3000-100-1-true-1",
    ...
    for workload_name, exps_per_label in exps_per_label_per_setting.items():
        ...
        for label, exps in exps_per_label.items():
            ...
            for exp_name in exps:
                ...
                f.write(exp_name + " " + str(0) + " " + str(latency_bar) + " " + str(success_rate) + " " + str(weighted_success_rate) + " " + str(avg_parallelism) + "\n")
                f.close()
