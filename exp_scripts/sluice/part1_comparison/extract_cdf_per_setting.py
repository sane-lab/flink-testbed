...
def plot_latency_curves(latency_curves, latency_limit, output_dir, start_time, exp_length, workload_name: str, ax, ylabel_flag):
    ...
    ax.set_title(workload_name, y=-0.6, fontsize=FONT_SIZE + 8)
...
def plot_parallelism_curves(parallelism_curve, arrival_curve, output_dir, start_time, exp_length, workload_name: str, ax1, ax2, y1label_flag, y2label_flag):
    ...
    if y2label_flag:
        ax2.set_ylabel('Arrival Rate', fontsize=FONT_SIZE, labelpad=3)
...
def main():
    ...
    fig, axs = plt.subplots(3, 3, figsize=(21, 7), layout='constrained', gridspec_kw={'height_ratios': [1, 2, 3]}) #[1, 2, 2]})
    index = 0
    legend_elements = []
