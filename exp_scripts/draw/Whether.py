...
#    axes.set_xticklabels(xlabels)
    ...
    #axes.set_yscale('log')
    #axes.set_ylim(100, 5000)
    #axes.set_yticks([100, 500, 1000, 2000, 5000])  # np.arange(0, 1000, 200))
...
def drawAll(resultsPerCurve, outputDir, curves):
    ...
    fig, axs = plt.subplots(3, CURVENUM, figsize=(5 * CURVENUM, 9), layout='constrained') #(24, 9)
    ...
    fig.legend(lines, labels, bbox_to_anchor=(0.5, 1.1), loc='upper center', ncol=4)
    import os
    if not os.path.exists(outputDir):
        os.makedirs(outputDir)
    plt.savefig(outputDir + "AllinOne_whether.pdf", bbox_inches='tight')
    plt.close(fig)
