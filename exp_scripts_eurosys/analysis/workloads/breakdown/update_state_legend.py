...
from analysis.config.default_config import timers_plot, per_task_rate, parallelism, per_key_state_size, \
    replicate_keys_filter, state_access_ratio, max_parallelism, FILE_FOLER, FIGURE_FOLDER, order_function, zipf_skew, sync_keys
from analysis.config.general_utilities import DrawFigureV4, DrawLegend, breakdown_total
...
def ReadFile(repeat_num = 1):
    ...
    for line in lines:
        ...
        if timer[0] == "++++++replicationTimer" and "Timer" in prev_timer and prev_timer != "++++++replicationTimer":
            ...
            y[0].append(cur_replication_time / counter)
            cur_replication_time = 0
            counter = 0
        # elif timer[0] != "++++++replicationTimer" and "Timer" in timer[0]:
            ...
            # y[1].append(cur_replication_time)
            # cur_replication_time = 0
        if "Timer" in timer[0]:
            prev_timer = timer[0]
        if timer[0] == "++++++replicationTimer":
            cur_replication_time += int(timer[1][:-3])
            counter += 1
    ...
    for line in lines:
        ...
        if timer[0] == "++++++replicationTimer" and "Timer" in prev_timer and prev_timer != "++++++replicationTimer":
            # migration end, compute the completion time
            y[1].append(cur_replication_time / counter)
            cur_replication_time = 0
            counter = 0
        # elif timer[0] != "++++++replicationTimer" and "Timer" in timer[0]:
            # migration start, sum up all timers in between
    ...
    for line in lines:
        ...
        if timer[0] == "++++++replicationTimer" and "Timer" in prev_timer and prev_timer != "++++++replicationTimer":
            # migration end, compute the completion time
            y[2].append(cur_replication_time / counter)
            cur_replication_time = 0
            counter = 0
        # elif timer[0] != "++++++replicationTimer" and "Timer" in timer[0]:
            # migration start, sum up all timers in between
    ...
    for line in lines:
        ...
        if timer[0] == "++++++replicationTimer" and "Timer" in prev_timer and prev_timer != "++++++replicationTimer":
            # migration end, compute the completion time
            y[3].append(cur_replication_time / counter)
            cur_replication_time = 0
            counter = 0
        # elif timer[0] != "++++++replicationTimer" and "Timer" in timer[0]:
            # migration start, sum up all timers in between
...
def draw():
    ...
    DrawLegend(legend_labels, "update_state_legend")
